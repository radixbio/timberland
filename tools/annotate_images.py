...
imagename_pat = r'image\s*=\s*"(?:{0}|{1})"'.format(
...
def get_digest(templatefile, imagename, image_available):
    ...
    if not image_available:
        ...
        proc = subprocess.Popen(
            ["docker", "pull", imagename], stdout=subprocess.PIPE
        ).stdout.read()
        if not proc.splitlines(keepends=False):
            print(
                "[ERROR] No output from docker for "
                + imagename
                + " on templatefile "
                + templatefile
            )
    ...
    shortname = imagename.split(":")[
        0
    ]  # Looking for image name with version tag suppresses digest, for some reason?
    cmd = [
        "docker",
        "images",
        "--digests",
        "--filter",
        "reference=%s" % shortname,
        "--format",
        "{{.Digest}}",
    ]
    digest_output = (
    ...
    if digstr and "<none>" not in digstr:
        res = (
            shortname + "@" + digstr
        )  # should be the full docker image name without tag with sha256 version digest
        return res
...
def get_available_images():
    lines = (
        subprocess.Popen(
            ["docker", "images", "--format", "{{.Repository}}"], stdout=subprocess.PIPE
        )
        .stdout.read()
        .decode("utf-8")
        .split("\n")
...
def docker_is_available():
    try:
        out = subprocess.Popen(
            ["docker", "--version"], stdout=subprocess.DEVNULL
        ).wait()
        return bool(out)
...
def annotate_images(templatefile, outputfile, dot_git_dir):
    ...
    if "ref: refs/heads" not in branch_name:
        ...
        branches = (
            subprocess.Popen(
                ["git", "--git-dir", dot_git_dir, "branch", "--contains", branch_name],
    ...
    branch_name = branch_name.replace(
        "/", "-"
    )  # Formatting to match workspace-status.sh

    if not branch_name:
        print(
            "[WARN] %s: Failed to find valid branch name in build repo, using 'master' (.git/HEAD contains '%s')"
            % (os.path.basename(templatefile), branch_name)
        )

        newline_indices = [
            m.end() - 2 for m in re.finditer(r"{CURRENT_BRANCH}.*\n", text)
        ]
        diff = 0
