...
case class RetoolPGKafkaConnector() extends Job {
  val name = "retool-pg-kafka-connector"
  ...
  val groups = List(RetoolPGKafkaConnectorGroup)

  object RetoolPGKafkaConnectorGroup extends Group {
    val name = "connector"
    val count = 1
    ...
    object ConnectorTask extends Task {
      val name = "retool-postgres-kafka-connector"
      val env = None
      ...
      object startTemplate extends Template {
        override val source =
          "/opt/radix/timberland/nomad/connect/postgres_source.sh".some
  ...
  def jobshim() = JobShim(name, RetoolPGKafkaConnector().assemble)
}