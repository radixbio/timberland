        ...
        os.proc("/etc/confluent/docker/run")
          .call(
            env = sys.env
              ...
              .updated("KAFKA_LISTENERS", s"INSIDE://0.0.0.0:$interBrokerPort,OUTSIDE://0.0.0.0:${sys.env("NOMAD_PORT_kafka")}")
              .updated("KAFKA_ADVERTISED_LISTENERS", s"INSIDE://$weave:$interBrokerPort,OUTSIDE://${sys.env("NOMAD_ADDR_kafka")}")
