...
object wan {
  ...
  def wanJoin(leaderNode: Option[String], username: Option[String], password: Option[String], publicAddr: String): IO[Unit] = for {
    remoteAddr <- IO {
  ...
  private def deployMeshGatewayJobspec(token: String, dc: String, wanAddr: String): IO[Unit] = blaze.use { client =>
    val job = JobShim("mesh-gateway", Job(
      datacenters = List(dc),
      `type` = "system",
      vault = Some(Vault(
      ...
      group = List(GroupShim("gateway", Group(
        network = Some(Network(
          ports = List.empty,
          mode = Some("host")
        )),
        task = List(TaskShim("gateway", Task(
          template = Some(List(TemplateShim(Template(
            data = Some(List(
              """{{with secret "secret/tokens/actor-token"}}""", "{{if .Data.data.token}}",
          ...
          driver = "raw_exec",
          config = Some(DockerConfig( // Not actually a docker config
            dns_servers = None,
            command = Some((RadPath.persistentDir / "consul" / "consul").toString()),
            args = Some(List(
              "connect", "envoy", "-token", "${ACCESS_TOKEN}",
              "-ca-file", (RadPath.runtime / "certs" / "ca" / "combined.pem").toString(),
              "-client-cert", (RadPath.runtime / "certs" / "cli" / "cert.pem").toString(),
              "-client-key", (RadPath.runtime / "certs" / "cli" / "key.pem").toString(),
              "-gateway", "mesh", "-expose-servers", "-register", "-service", s"gateway-$dc",
              "-address", "127.0.0.1:8443", "-wan-address", s"$wanAddr:8443"
            ))
          ))
        )))
      )))
  ...
  private def rebootstrapNomad(): IO[Unit] = {
    ...
    val bootstrapCmd = List(
      RadPath.nomadExec.toString(), "acl", "bootstrap",
      "-tls-skip-verify", "-address", "https://127.0.0.1:4646",
  ...
  private def replicateRootCerts(remoteAddr: String, remoteAuth: AuthTokens, dc: String, localDc: String): IO[Unit] = {
    for {
      ...
      cmd = os.proc(List(
        consulBin, "exec",
        "-http-addr", s"https://$remoteAddr:8501",
        "-token", remoteAuth.consulNomadToken,
        "-ca-file", caFile.toString(),
        "-client-cert", certFile.toString(),
        "-client-key", keyFile.toString(),
        "-"
      ))
  ...
  private def getRemoteCerts(vaultAddress: String, vaultToken: String, dcName: String): IO[CertificateResponse] = {
    ...
    val vault = new VaultSession[IO](authToken = Some(vaultToken), baseUrl = vaultUri)(IO.ioConcurrentEffect, insecureBlaze)
