...
final class Http4sConsulClient[F[_]](
  ...
  private implicit val listKvGetResultDecoder
    : EntityDecoder[F, List[KVGetResult]] = jsonOf[F, List[KVGetResult]]
  private implicit val listServicesDecoder
    : EntityDecoder[F, Map[String, ServiceResponse]] =
    jsonOf[F, Map[String, ServiceResponse]]
  private implicit val listHealthChecksDecoder
    : EntityDecoder[F, List[HealthCheckResponse]] =
    jsonOf[F, List[HealthCheckResponse]]
  private implicit val listHealthNodesForServiceResponseDecoder
    : EntityDecoder[F, List[HealthNodesForServiceResponse]] =
  ...
  private def extractHeaderValue(header: String,
                                 response: Response[F]): F[String] = {
    response.headers.get(header.ci) match {
      case Some(header) => F.pure(header.value)
      case None =>
        F.raiseError(
          new NoSuchElementException(
            s"Header not present in response: $header"))
    }
  }
  ...
  private def extractConsulHeaders(response: Response[F]): F[ConsulHeaders] = {
    for {
      ...
      lastContact <- extractHeaderValue("X-Consul-LastContact", response).map(
        _.toLong)
      knownLeader <- extractHeaderValue("X-Consul-KnownLeader", response).map(
        _.toBoolean)
  ...
  /**
    ...
    */
  private def extractQueryResponse[A](response: Response[F])(
      implicit d: EntityDecoder[F, A]): F[QueryResponse[A]] = response match {
    case Successful(_) =>
      for {
        headers <- extractConsulHeaders(response)
        decodedBody <- response.as[A]
      } yield {
        QueryResponse(decodedBody,
                      headers.index,
                      headers.knownLeader,
                      headers.lastContact)
      }
  ...
  def kvGet(
    ...
    for {
      ...
      response <- client.fetch[QueryResponse[List[KVGetResult]]](req) {
        response: Response[F] =>
        ...
        addConsulToken(
          Request(
            uri = (baseUri / "v1" / "kv" / key)
              ...
              .+??("index", index)
              .+??("wait", wait.map(Interval.toString)))))
      response <- client.fetch[QueryResponse[Option[Array[Byte]]]](req) {
        response: Response[F] =>
          response.status match {
            case status @ (Status.Ok | Status.NotFound) =>
              for {
                ...
                value <- if (status == Status.Ok)
                  response.body.compile.to[Array].map(Some(_))
                else F.pure(None)
              } yield {
                QueryResponse(value,
                              headers.index,
  ...
  def kvSet(key: Key, value: Array[Byte]): F[Unit] =
    for {
      ...
      req <- PUT(uri = baseUri / "v1" / "kv" / key, value)
        .map(addConsulToken)
        .map(addCreds)
  ...
  def kvDelete(key: Key): F[Unit] = {
    val req = addCreds(
      addConsulToken(
        Request(Method.DELETE, uri = (baseUri / "v1" / "kv" / key))))
        ...
        addConsulToken(
          Request(
            uri = (baseUri / "v1" / "health" / "checks" / service)
              ...
              .+??("index", index)
              .+??("wait", wait.map(Interval.toString)))))
      response <- client.fetch[QueryResponse[List[HealthCheckResponse]]](req)(
  ...
  def healthChecksInState(
    ...
    for {
      ...
      req = addCreds(
        addConsulToken(
          Request(
            uri = (baseUri / "v1" / "health" / "state" / HealthStatus.toString(
              state))
              .+??("dc", datacenter)
              .+??("near", near)
              .+??("node-meta", nodeMeta)
  ...
  def healthNodesForService(
    ...
    for {
      _ <- F.delay(
        log.debug(s"fetching nodes for service $service from health API"))
      req = addCreds(
        addConsulToken(
          Request(
            uri = (baseUri / "v1" / "health" / "service" / service)
              .+??("dc", datacenter)
              .+??("near", near)
              .+??("node-meta", nodeMeta)
              .+??("tag", tag)
      ...
      response <- client
        .fetch[QueryResponse[List[HealthNodesForServiceResponse]]](req)(
          extractQueryResponse)
    } yield {
  ...
  def agentRegisterService(
    ...
    for {
      _ <- F.delay(
        log.debug(s"registering $service with json: ${json.toString}"))
      req <- PUT(baseUri / "v1" / "agent" / "service" / "register", json)
        .map(addConsulToken)
        .map(addCreds)
      response <- client.expectOr[String](req)(handleConsulErrorResponse)
    } yield
      log.debug(s"registering service $service resulted in response $response")
  }
  ...
  def agentDeregisterService(id: String): F[Unit] = {
    val req = addCreds(
      addConsulToken(
        Request(
          Method.PUT,
          uri = (baseUri / "v1" / "agent" / "service" / "deregister" / id))))
    for {
  ...
  def agentListServices(): F[Map[String, ServiceResponse]] = {
    for {
      ...
      req = addCreds(
        addConsulToken(Request(uri = (baseUri / "v1" / "agent" / "services"))))
      services <- client.expectOr[Map[String, ServiceResponse]](req)(
        handleConsulErrorResponse)
    } yield {
  ...
  def agentEnableMaintenanceMode(id: String,
    ...
    for {
      _ <- F.delay(
        log.debug(s"setting service with id $id maintenance mode to $enable"))
      req = addCreds(
        addConsulToken(
          Request(Method.PUT,
                  uri =
                    (baseUri / "v1" / "agent" / "service" / "maintenance" / id)
                      .+?("enable", enable)
                      .+??("reason", reason))))
      response <- client.expectOr[String](req)(handleConsulErrorResponse)
    } yield
      log.debug(
        s"setting maintenance mode for service $id to $enable resulted in $response")
  }
}
