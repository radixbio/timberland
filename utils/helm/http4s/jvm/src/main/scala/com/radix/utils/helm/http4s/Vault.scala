...
class Vault[F[_]: ConcurrentEffect](authToken: Option[String], baseUrl: Uri, blazeClient: Client[F]) extends VaultInterface[F] {
   ...
   */
  private def errorResponseHandler(response: Response[F]): F[Throwable] = response
    .body
  ...
  override def sysInit(req: InitRequest): F[Either[VaultError, InitResponse]] =
    submitRequest(Request[F](method = POST, uri = baseUrl / "v1" / "sys" / "init", headers = baseHeaders).withEntity(req.asJson))

  override def sysInitStatus: F[Either[VaultError,Boolean]] =
    submitRequest[io.circe.Json](Request[F](method = GET, uri = baseUrl / "v1" / "sys" / "init", headers = baseHeaders)).map(_.flatMap(_.hcursor.downField(("initialized")).as[Boolean].leftMap(_ => VaultErrorMalformedResponse())))


  override def unseal(req: UnsealRequest): F[Either[VaultError, UnsealResponse]] =
    submitRequest(Request[F](method = PUT, uri = baseUrl / "v1" / "sys" / "unseal", headers = baseHeaders).withEntity(req.asJson))

  override def registerPlugin(pluginType: PluginType, name: String, req: RegisterPluginRequest): F[Either[VaultError, Unit]] =
    submitRequestNoResponse(Request[F](method = PUT, uri = baseUrl / "v1" / "sys" / "plugins" / "catalog" / pluginType.toString / name, headers = baseHeaders).withEntity(req.asJson))

  override def enableSecretsEngine(pluginPath: String, req: EnableSecretsEngine): F[Either[VaultError, Unit]] =
    submitRequestNoResponse(Request[F](method = POST, uri = appendPath(baseUrl / "v1" / "sys" / "mounts", pluginPath), headers = baseHeaders).withEntity(req.asJson))

  override def authCodeUrl(pluginPath: String, req: AuthCodeUrlRequest): F[Either[VaultError, AuthCodeUrlResponse]] =
    submitRequest(Request[F](method = PUT, uri = appendPath(baseUrl / "v1", pluginPath) / "config" / "auth_code_url", headers = baseHeaders).withEntity(req.asJson))

  override def updateCredential(pluginPath: String, credentialName: String, req: UpdateCredentialRequest): F[Either[VaultError, Unit]] =
    submitRequestNoResponse(Request[F](method = PUT, uri = appendPath(baseUrl / "v1", pluginPath) / "creds" / credentialName, headers = baseHeaders).withEntity(req.asJson))
  ...
  override def createSecret(name: String, req: CreateSecretRequest): F[Either[VaultError, CreateSecretResponse]] =
    submitRequest(Request[F](method = POST, uri = baseUrl / "v1" / "secret" / "data" / name, headers = baseHeaders).withEntity(req.asJson))
