package com.radix.utils.helm.js
...
abstract class JSConsulClient[F[_]](url: String)(implicit F: Effect[F], cs: ContextShift[IO])
  ...
  override def apply[A](op: ConsulOp[A]): F[A] = op match {
    ...
    case ConsulOp.KVSet(key, casNewKey)          => kvSet(key, casNewKey)
  ...
  override def kvSet(key: Key,
                     value: Array[Byte],
                     session: UUID): F[Boolean] = {
    val url = URL + "/v1/kv/" + key
    val req = HttpRequest()
      .withURL(url)
      .withMethod(Method.PUT)
      .withQueryParameter("acquire", session.toString)
    for {
      _ <- F.delay(logger.debug(s"setting consul key $key to $value"))
      resp <- F.liftIO(
        IO.fromFuture(IO(req.put(ByteBufferBody(ByteBuffer.wrap(value))))))
      ...
      logger.debug(
        s"setting consul key $key with session $session resulted in response $resp")
      resp.body.stripLineEnd.toBoolean
    }
  }
  override def kvSet(key: Key,
                     last: KVGetResult,
                     value: Array[Byte]): F[Boolean] = {
    val url = URL + "/v1/kv/" + key
    val req = HttpRequest()
      .withURL(url)
      .withMethod(Method.PUT)
      .withQueryParameter("cas", last.modifyIndex.toString)
    for {
      _ <- F.delay(logger.debug(s"setting consul key $key to $value"))
      resp <- F.liftIO(
        IO.fromFuture(IO(req.put(ByteBufferBody(ByteBuffer.wrap(value))))))
      ...
      logger.debug(
        s"setting consul key $key with CAS resulted in response $resp")
      resp.body.stripLineEnd.toBoolean
    }
  }
  override def kvSet(key: Key,
    ...
    val req = HttpRequest()
      .withURL(url)
      .withMethod(Method.PUT)
      .withQueryParameter("cas", modifyIndex.toString)
    for {
      _ <- F.delay(logger.debug(s"setting consul key $key to $value"))
      resp <- F.liftIO(
        IO.fromFuture(IO(req.put(ByteBufferBody(ByteBuffer.wrap(value))))))
  ...
  def sessionCreate(name: String,
                    ...
                    behavior: Option[String]): F[UUID] = ???

  def getSessionInfo(session: UUID): F[SessionResponse] = {
    val url = "v1/session/info/" + session
    val req = HttpRequest()
      .withURL(url)
      .withMethod(Method.GET)
    for {
      _ <- F.delay(logger.debug(s"Getting session info from $session"))
      resp <- F.liftIO(IO.fromFuture(IO(req.send())))
      parsed <- F.liftIO(IO.fromEither(resp.body.decodeEither[SessionResponse].leftMap(err => new RuntimeException(err))))

    } yield {
      logger.debug(
        s"Session info for session $session resulted in response $parsed")
     parsed
    }
  }
}
